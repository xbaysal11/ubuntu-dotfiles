...
exports.default = () => __awaiter(this, void 0, void 0, function* () {
    ...
    if (!material_theme_1.isMaterialTheme(themeLabel)) {
        return deferred.resolve();
    }
    const DEFAULTS = fs_1.getDefaultsJson();
    const PKG = fs_1.getPackageJson();
    const MT_SETTINGS = vscode_1.getMaterialThemeSettings();
    const materialIconVariantID = material_theme_1.getThemeIconVariant(DEFAULTS, themeLabel);
    const currentThemeIconsID = vscode_1.getCurrentIconsID();
    ...
    const themeIconsPath = icons_1.getIconsPath(PKG, newThemeIconsID);
    // Actual json file of the icons theme (eg. Material-Theme-Icons-Darker.json)
    const theme = fs_1.getIconsVariantJson(themeIconsPath);
    ...
    const themePath = fs_1.getAbsolutePath(themeIconsPath);
    // Write changes to current JSON icon
    fs.writeFile(themePath, JSON.stringify(theme), { encoding: 'utf-8' }, (err) => __awaiter(this, void 0, void 0, function* () {
        if (err) {
            deferred.reject(err);
            return;
        }
